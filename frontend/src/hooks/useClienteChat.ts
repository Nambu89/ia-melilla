...
/**
 * Variante authenticated del useDemoChat. Reutiliza el mismo reducer y parser
 * SSE pero toma el token de AuthContext (sesión real del cliente) en lugar
 * de las credenciales demo precreadas en variables de entorno.
 *
 * No hace login: asume que el llamante está dentro de RequireAuth y por tanto
 * AuthContext ya tiene una sesión válida. Si la sesión expira mientras el
 * usuario chatea, el backend devolverá 401 y se mostrará error con botón de
 * reintentar (que vuelve a leer el token del contexto).
 */

export type Message = { role: "user" | "assistant"; content: string };

export type ChatStatus = "ready" | "asking" | "streaming" | "error" | "no_session";
...
export function useClienteChat() {
...
/**
 * Mismo parser SSE que useDemoChat — copia local para mantener cero acoplamiento
 * entre las dos variantes (demo público vs cliente authenticated). Si en el
 * futuro hay que cambiar el formato del stream, ambos hooks se actualizan en
 * paralelo.
 */
